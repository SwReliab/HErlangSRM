...
List Cemest(NumericVector t, IntegerVector i, IntegerVector j, IntegerVector n,
  ...
  double llf = Cestep(t, i, j, n, omega, p, lam, a, mu, b, em, ex, es, g, fx);
  Cmstep(em, ex, es, omega, p, lam, a, mu, b);
  int iter = 1;
  ...
  while (true) {
    double prev = llf;
    llf  = Cestep(t, i, j, n, omega, p, lam, a, mu, b, em, ex, es, g, fx);
    Cmstep(em, ex, es, omega, p, lam, a, mu, b);
...
NumericVector Cmvfc(NumericVector t, List model, int gn = 15, double geps = 1.0e-8) {
  const double omega = model["omega"];
  NumericMatrix p = model["p"];
  NumericVector lam = model["lam"];
  IntegerVector a = model["a"];
  ...
  for (int k=0; k<t.length(); k++) {
    for (int ii=0; ii<a.length(); ii++) {
      for (int jj=0; jj<b.length(); jj++) {
        result[k] += omega * p(ii,jj) * ff1(prevt, t[k], lam[ii], a[ii], mu[jj], b[jj], g, fx);
      }
    }
    for (int l=k+1; l<t.length(); l++) {
      for (int ii=0; ii<a.length(); ii++) {
        for (int jj=0; jj<b.length(); jj++) {
          result[l] += omega * p(ii,jj) * ff2(prevt, t[k], t[l-1], t[l], lam[ii], a[ii], mu[jj], b[jj], g, fx);
        }
      }
    }
    prevt = t[k];
